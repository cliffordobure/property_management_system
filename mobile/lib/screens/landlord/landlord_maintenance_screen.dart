...
class LandlordMaintenanceScreen extends StatefulWidget {
  ...
  @override
  State<LandlordMaintenanceScreen> createState() => _LandlordMaintenanceScreenState();
}
    ...
    return RefreshIndicator(
      ...
      child: ListView.builder(
        ...
        itemBuilder: (context, i) {
          ...
          final title = m['title']?.toString() ?? m['description']?.toString() ?? 'Request';
          final status = m['status']?.toString() ?? '—';
          final date = _formatDate(m['requestedDate']);
          final property = m['propertyId'];
          final propName = property is Map ? property['propertyName']?.toString() : null;
          final tenant = m['tenantId'];
          final tenantName = tenant is Map ? '${tenant['firstName'] ?? ''} ${tenant['lastName'] ?? ''}'.trim() : null;
          return Card(
            margin: const EdgeInsets.only(bottom: 12),
            child: ListTile(
              title: Text((title.length > 45 ? '${title.substring(0, 45)}...' : title)),
              subtitle: Text([date, status, if (propName != null) propName, if (tenantName != null) tenantName].join(' · ')),
              trailing: Container(
